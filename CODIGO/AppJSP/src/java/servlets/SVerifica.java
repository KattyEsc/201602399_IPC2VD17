 ...
 * @author Katty
 */
@WebServlet(name = "SVerifica", urlPatterns = {"/SVerifica"})
public class SVerifica extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Empleado emp= verifica(nickname,pass);
        if(emp!=null){
            HttpSession sesion= request.getSession();
            sesion.setAttribute("validUsuario", emp);
    ...
    private Empleado verifica(java.lang.String nickname, java.lang.String password) {
        // Note that the injected javax.xml.ws.Service reference as well as port objects are not thread safe.
        // If the calling of port operations may lead to race condition some synchronization is required.
        ws.WSEmpleado port = service.getWSEmpleadoPort();
        return port.verifica(nickname, password);
    }
